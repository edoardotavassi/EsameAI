...
class Problem:
    ...
    def __init__(self, initial, n=0, goal=None):
...
class Node:
    ...
    def __repr__(self):
        ...
        for i in self.state:
            if i[1]:
                ...
                string += str(i[0])+" "
            if count == matrix_dimension:
                string += "\n"
                count = 0
            count +=1
        string += "Cost: "
        string += str(self.path_cost)
        string += "\n------------------"
        return string
    ...
    def expand(self, problem):
        ...
        for action in problem.actions(self.state):
            out.append(self.child_node(problem, action))
        return out

    def child_node(self, problem, action):
        """[Figure 3.10]"""
        next_state = problem.result(self.state, action)
        #print("Child Node:" + str(action))
        #printState(next_state, 4, [])
        next_node = Node(
            next_state,
            self,
            action,
            problem.path_cost(self.path_cost, action),
        )
        return next_node
